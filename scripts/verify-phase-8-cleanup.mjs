...
const recordPath = process.argv[2] ?? process.env.GODOT_MCP_PHASE8_CLEANUP_RECORD;
const residuePattern = /(?:^|\/)(?:(?:\.consuming-)?runtime-[^/]+\.(?:json|lease)|editor-launch-[^/]+\.json)$|godot-mcp-phase8|phase-8-(?:scenario|visual|baseline)/i;
const offenders = [];
...
if (recordPath && !(await exists(recordPath))) {
  ...
  for (const pid of Array.isArray(record.pids) ? record.pids : []) {
    ...
    try { process.kill(pid, 0); offenders.push(`live recorded PID: ${pid}`); } catch {}
  }
...
if (offenders.length > 0) {
  process.stderr.write(`Phase 8 cleanup verification failed:\n${offenders.sort().join("\n")}\n`);
  process.exit(1);
}
process.stdout.write("Phase 8 fixture, runtime, scenario, baseline, descriptor, lease, and listener cleanup verified\n");
