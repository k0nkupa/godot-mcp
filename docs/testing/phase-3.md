...
`godot_runtime` owns at most one generation and supports launch, status, bounded tree/node/log reads, typed waits, pause, one-to-120-frame step, resume, and stop. Scene paths stay under `res://`; node paths are relative and reject traversal and subnames. Tree responses are limited to depth 32 and 1,000 nodes, node results to 128 properties and signals, logs to 500 records, and waits to 30 seconds. Property-pattern waits use a restricted 64-character regex subset against at most 4,096 subject characters so caller-controlled matching cannot monopolize the runtime main thread.
