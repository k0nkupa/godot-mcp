...
Deterministic receipts certify the schedule of MCP-injected events; they do not suppress or record concurrent platform input delivered to a windowed Godot runtime. The closed event factory marks its internally constructed `InputEvent` objects with private metadata, and the release fixture uses that diagnostic provenance to build a trace-scoped action/key digest while separately retaining an all-input digest for mouse, touch, gesture, joypad, and ambient-input coverage. The marker is not authentication or a protocol security boundary. Release comparisons also assert exact delivered frames. Failure evidence records digest values and changed property names only, never raw action names, keycodes, coordinates, or trace payloads.
