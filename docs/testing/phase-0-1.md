...
The E2E uses a disposable copy of `fixtures/godot-4.7`. It initializes the addon, launches a real headless editor, connects through the MCP SDK’s stdio transport, waits for authenticated attachment, discovers exactly four tools, shuts down, disables and uninstalls the addon, then requires a zero diff from the original project.
