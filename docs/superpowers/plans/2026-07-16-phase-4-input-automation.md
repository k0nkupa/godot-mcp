...
- Signed protocol values remain canonical JSON v1 safe integers. Normalized coordinates, strengths, pressures, tilts, magnification, and gamepad axes use integer millionths and are converted to Godot floats only inside the authenticated harness; viewport/embedder pixel coordinates remain integers in `[-8192, 8192]`. Touch indices are 0–9 and gamepad devices are 0–7.
