...
3. The certified editor launcher writes and the addon consumes a ten-second, owner-only, project-and-port-bound startup attestation, proving the shared debugger/DAP collision was launcher-established rather than asserted only by copyable user arguments. Containment, regular-file shape, permissions, and size are validated before the addon reads or deletes the selected path. The control plane then proves the editor owns the loopback debugger listener, launches the fixed runtime harness, and waits for the existing one-use descriptor and signed hello exchange to authenticate the owned child PID and unique `EditorDebuggerSession`. The runtime starts an independent lease watchdog thread before authentication; it remains active while the main thread is debugger-stopped and terminates only its own process after lease expiry or editor revocation. A later ambiguous debugger session revokes that lease instead of issuing a raw PID kill.
4. Breakpoints and execution control use the bound `EditorDebuggerSession`. Stack and variable evidence is captured inside the authenticated runtime with `Engine.capture_script_backtraces(true)` and returned through the existing signed, sequenced, deadline-bound runtime command channel. Godot returns one candidate per registered script language rather than a thread identifier; the stopped-command path therefore selects the GDScript candidate with the most project-local non-addon frames and rejects addon-only candidates.
5. Performance evidence uses the same authenticated runtime channel with public `Performance`, `EngineProfiler`, and `RenderingServer` APIs. GPU timestamps are explicitly unsupported until actual rendered work can be bracketed across frame boundaries.
