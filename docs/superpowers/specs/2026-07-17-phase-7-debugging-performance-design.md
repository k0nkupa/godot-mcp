...
3. The certified editor launcher writes and the addon consumes a ten-second, owner-only, project-and-port-bound startup attestation, proving the shared debugger/DAP collision was launcher-established rather than asserted only by copyable user arguments. The control plane then proves the editor owns the loopback debugger listener, launches the fixed runtime harness, and waits for the existing one-use descriptor and signed hello exchange to authenticate the owned child PID and unique `EditorDebuggerSession`. It certifies the independently verified child PID back to the editor so an editor-side lease watchdog can terminate that exact child even while the game main thread is stopped in the debugger. A later ambiguous debugger session terminates that certified child while retaining watchdog identity until the stop is observed.
