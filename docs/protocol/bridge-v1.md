...
`runtime.prepare` returns the editor PID, debugger port, and `debugTransport: "authenticated-editor-session"`. Before returning that metadata, the addon stops Godot 4.7's unauthenticated native DAP server and holds its configured loopback port with an inert guard. Preparation fails if the guard is absent. The control plane proves the recorded editor PID owns the debugger listener, then launches the child. Debugger state is created only after the one-use descriptor handshake authenticates the owned child PID and identifies its sole active editor debugger session. Every debugger operation rechecks that binding and fails closed on ambiguity.
