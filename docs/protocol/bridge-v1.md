...
`runtime.prepare` returns the editor PID, debugger port, and `debugTransport: "authenticated-editor-session"`. The certified CLI launcher assigns that debugger and Godot 4.7's unauthenticated native DAP server to the same loopback port; the authenticated debugger binds first, so native DAP never becomes reachable. The addon stops the inactive DAP plugin and preparation fails unless the secure-launch marker and shared-port identity are present. The control plane proves the recorded editor PID owns the debugger listener, then launches the child. Debugger state is created only after the one-use descriptor handshake authenticates the owned child PID and identifies its sole active editor debugger session. Every debugger operation rechecks that binding and fails closed on ambiguity.
...
Debugger operations are carried by the existing `godot_runtime` tool: set/clear breakpoints, status, wait, pause, continue, step over/into, stack, variables, children, and selector watches. Breakpoints are canonical project-local GDScript locations outside the addon. Runtime frame and variable IDs are replaced with opaque tokens bound to run ID, runtime generation, authenticated debugger generation, and stop sequence. Tokens become stale on execution, a new stop, reconnect, shutdown, or generation change. Watches traverse exact scope/name/index selectors in bounded captured variable trees and never evaluate expressions.

Performance operations on the same tool are monitor snapshot, profile start/status/cancel/result. The harness samples public `Performance`, `EngineProfiler`, `RenderingServer`, and `RenderingDevice` APIs only. One job runs at a time for 100 ms–30 seconds with an interval of 1–120 frames, at most eight requested groups, 128 metrics, 2,048 retained samples, and four MiB of complete wire-encoded terminal evidence. Requested built-in groups and EngineProfiler ticks take priority over custom monitors at the metric cap. Terminal evidence records completeness, cancellation/failure reason, explicit metric-truncation metadata, engine metadata, aggregates, optional bounded samples, current-job GPU timestamps, and a canonical SHA-256. Audit stores only bounded operation/count/state/digest metadata, never debugger values, watches, monitor samples, or raw profile evidence.
