...
describe("Phase 7 runtime debugging schemas", () => {
  it("accepts the closed debugger operation surface", () => {
    ...
    expect(RuntimeDebugOperationInputSchema.parse({ operation: "debug_variables", handle, frameToken: opaqueFrame, scope: "locals" })).toMatchObject({ offset: 0, limit: 100 });
  ...
  it("enforces debugger bounds and strict objects", () => {
    ...
    expect(() => RuntimeDebugOperationInputSchema.parse({ operation: "debug_variables", handle, frameToken: opaqueFrame, scope: "locals", limit: 257 })).toThrow();
  ...
  it("validates opaque debugger tokens", () => {
    expect(DebugFrameTokenSchema.parse(opaqueFrame)).toBe(opaqueFrame);
