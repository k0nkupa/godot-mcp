...
func _init() -> void:
	...
	logger.record_for_test("error", "{\"path\":\"/Volumes/Quoted/private.png\"}")
	logger.record_for_test("error", "source=file:///Users/example/private.gd safe=https://example.test/docs res=res://main.tscn")
	...
	assert("/Volumes/Quoted" not in JSON.stringify(records))
	assert("/Users/example/private.gd" not in JSON.stringify(records))
	assert("https://example.test/docs" in JSON.stringify(records))
	assert("res://main.tscn" in JSON.stringify(records))
